...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/jmpezzHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
search_gifs = db.Table('search_gifs', db.Column('gifs', db.Integer, db.ForeignKey('gifs.id')), db.Column('search_term', db.Integer, db.ForeignKey('search_term.id')))
user_collection = db.Table('user_collection', db.Column('gifs', db.Integer, db.ForeignKey('gifs.id')), db.Column('personal_gifs', db.Integer, db.ForeignKey('personal_gifs.id')))
...
def get_gifs_from_giphy(search_string):
    ...
    get_data = requests.get(baseurl, params = params).json()
...
def get_or_create_gif(title, url):
    ...
    gifs = Gif.query.filter_by(title = title).first()
    if gifs:
        return gifs
    else:
        gifs = Gif(title = title, embedURL = url)
        db.session.add(gifs)
        db.session.commit()
        return gifs
...
def get_or_create_search_term(term):
    ...
    if search:
        ...
        for x in gifs:
            gifs2 = get_or_create_gif(x['title'], x['embed_url'])
            search.gifs.append(gifs2)
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    collection = PersonalGifCollection.query.filter_by(name = name, user = current_user.id).first()
    if collection:
        return collection
    else:
        collection = PersonalGifCollection(name = name, user = current_user.id)
        for x in gif_list:
...
def index():
    ...
    form = GifSearchForm()
    if form.validate_on_submit():
        search_obj = get_or_create_search_term(form.search.data)
        return redirect(url_for('search_results', search_term = search_obj.term))
...
def search_results(search_term):
    ...
    print ("Search term in gifs_searched", search_term)
    print ("Term", term)
...
def create_collection():
    ...
    if form.validate_on_submit():
        gif_list = []
        for x in form.gif_picks.data:
            gif = get_gif_by_id(x)
            gif_list.append(gif)
        collection = get_or_create_collection(form.name.data, current_user, gif_list)
...
def collections():
    ...
    collections = PersonalGifCollection.query.filter_by(user = current_user.id).all()
